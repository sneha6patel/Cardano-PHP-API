...
class Cardano
{
    ...
    const CERT_PATH = "./cardano-sl/state-wallet-mainnet/tls/client/client.pem";
    ...
    // Generate: Spending password (must be 32 characters) here: https://simplycalc.com/base16-encode.php
    // $body   : array(
    //              "operation" => "create",
    //              "backupPhrase" => "recipe have broom monkey apple orbit skull siege maze fence flame drill",
    //              "name" => "Wallet Name",
    //              "spendingPassword" => "3132333435363738393031323334353637383930313233343536373839303132"
    //           )
    ...
    public function createAddress(string $accountIndex, string $walletId, string $spendingPassword): array
    {
        return self::jsonDecode($this->altPost('/api/v1/addresses?spendingPassword='.$spendingPassword.'?accountIndex='.$accountIndex.'?walletId='.$walletId), true);
    }
    ...
    public function createNewTransaction(array $source, array $destination, string $groupingPolicy,  string $spendingPassword): array
    {

        return self::jsonDecode($this->post('/api/v1/transactions?&source='.$source.'&destination='.$destination.'&spendingPassword='.$spendingPassword));
    }
   ...
   //////////////////////////////
    ...
    // CURL Post
    private function post(string $endPoint, $postFields = [])
    {
        $ch = curl_init();
